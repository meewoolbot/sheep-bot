...
# ВЕСЬ СПИСОК ОВЕЧЕК (Старые + Новые)
RARITIES = {
...
async def sh(m: types.Message):
    ...
    # Чистый профиль: смайлик + название + точка
...
async def shear(cb: types.CallbackQuery):
    ...
    if u['shearing']:
        if now >= u['s_finish']:
            ...
            m, s = divmod(rem, 60)
            kb = InlineKeyboardBuilder().row(types.InlineKeyboardButton(text="⌛️ Проверить время", callback_data="shear")).row(types.InlineKeyboardButton(text="⬅️ Назад", callback_data="back"))
            try: await cb.message.edit_text(f"✂️ Стрижём твою овечку. ⏳ Процесс займет: {m} мин. {s} сек.", reply_markup=kb.as_markup())
            except: await cb.answer(f"⏳ Осталось: {m} мин. {s} сек.")
    elif now < u['harvest']:
...
async def sell(cb: types.CallbackQuery):
    ...
    if u['wool'] <= 0: await cb.answer("🐑 Мее! Дружок, постриги сначала свою овечку.", show_alert=True)
    else:
        val = u['wool'] * 10; u['balance'] += val; u['wool'] = 0; save_u(u)
        kb = InlineKeyboardBuilder().add(types.InlineKeyboardButton(text="⬅️ Назад", callback_data="back")).as_markup()
        await cb.message.edit_text(f"💰 Ты успешно продал всю шерсть! \nПолучено: {val} 🐾", reply_markup=kb)
...
async def open_e(cb: types.CallbackQuery):
    u = get_u(cb.from_user.id)
    if u['balance'] < 100: # Проверка тоже 100
