...
RARITIES = {
    ...
    "🔴 Мифическая": {"items": ["👼 Священная овечка", "👻 Призрачная овечка", "🕯️ Ритуальная овечка"], "w": 9},
...
async def shear(cb: types.CallbackQuery):
    ...
    if u['shearing']:
        if now >= u['s_finish']:
            ...
            kb = InlineKeyboardBuilder().row(types.InlineKeyboardButton(text="⌛️ Проверить время", callback_data="shear"))
