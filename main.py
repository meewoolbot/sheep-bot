...
# Твои редкости
RARITIES = {
    "🔵 Редкая": {"items": [("🏡 Деревенская овечка", "🐑")], "w": 50},
...
# --- БАЗА (чтобы ничего не пропадало) ---
def init_db():
...
app = Flask(__name__)
@app.route('/')
def h(): return "Sheep Farm Active"
...
async def sh(m: types.Message):
    ...
    timer = "✅ Шерсть готова к сбору." if now >= u['harvest'] else f"⏳ Шерсть будет готова к сбору через: {time.strftime('%H:%M:%S', time.gmtime(int(u['harvest']-now)))}"
    await m.answer(f"/sheep {u['skin']}.\n🐾 Копытца: {u['balance']}\n{timer}", reply_markup=main_kb())
...
async def shear(cb: types.CallbackQuery):
    ...
    if u['shearing']:
        if now >= u['s_finish']:
            ...
            await cb.answer("⏳ Процесс еще идет!", show_alert=True)
    elif now < u['harvest']:
...
async def open_e(cb: types.CallbackQuery):
    u = get_u(cb.from_user.id)
    if u['balance'] < 200: await cb.answer("❌ Недостаточно копытц!", show_alert=True)
    else:
        u['balance'] -= 200; r_l = list(RARITIES.keys()); rarity = random.choices(r_l, weights=[RARITIES[k]["w"] for k in r_l])[0]
        u['skin'] = random.choice(RARITIES[rarity]["items"])[0]; save_u(u)
        kb = InlineKeyboardBuilder().add(types.InlineKeyboardButton(text="⬅️ Назад", callback_data="back")).as_markup()
...
async def back(cb: types.CallbackQuery):
    ...
    await cb.message.edit_text(f"/sheep {u['skin']}.\n🐾 Копытца: {u['balance']}\n{t}", reply_markup=main_kb())
