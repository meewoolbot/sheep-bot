...
# СПИСОК ОВЕЧЕК (Оставил Редкие, Эпические, Легендарные, Мифические и Космические без стихийных)
RARITIES = {
    "🔵 Редкая": {"items": ["🏡 Деревенская овечка", "🏖️ Пляжная овечка", "💤 Сонная овечка"], "w": 50},
    "🟣 Эпическая": {"items": ["💥 Шизанутая овечка", "🎀 Милая овечка", "🍭 Карамельная овечка"], "w": 25},
    "🟡 Легендарная": {"items": ["🔥 Магмовая овечка", "💎 Бриллиантовая овечка", "🐚 Жемчужная овечка"], "w": 15},
    "🔴 Мифическая": {"items": ["👼 Священная овечка", "👻 Праздничная овечка", "🕯️ Ритуальная овечка"], "w": 9},
    "⚫️ Космическая": {"items": ["☢️ Радиоактивная овечка", "👁️‍🗨️ Пустотная овечка", "🧬 Мутагенная овечка"], "w": 1}
}
...
async def open_egg(cb: types.CallbackQuery):
    ...
    if u['balance'] < 100: await cb.answer("❌ Недостаточно копытц!", show_alert=True)
        ...
        u['balance'] -= 100
        r_l = list(RARITIES.keys()); rarity = random.choices(r_l, weights=[RARITIES[k]["w"] for k in r_l])[0]
