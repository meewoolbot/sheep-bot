...
def init_db():
    ...
    cursor = conn.cursor()
    # status: 0 - растет, 1 - готова к стрижке, 2 - в процессе стрижки (5 мин)
...
def handle_query(call):
    ...
    if call.data == 'shear':
        if u[7] == 0:
            ...
            bot.answer_callback_query(call.id, "⌛️ Овечка уже в процессе стрижки!")
        ...
        if (now - last_t) >= timedelta(minutes=5):
            ...
            bot.answer_callback_query(call.id, "Еще не закончили... Подожди немного!")

    elif call.data == 'market':
        text = f"🐑 Овечий рынок.\n💰 Курс: 1 🧶 = 10 🐾\n\nУ тебя сейчас: {u[2]} 🧶"
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton("💰 Продать шерсть", callback_data='sell'),
                   types.InlineKeyboardButton("⬅️ Назад", callback_data='back'))
        bot.edit_message_text(text, call.message.chat.id, call.message.message_id, reply_markup=markup)

    elif call.data == 'sell':
        if u[2] <= 0:
            bot.answer_callback_query(call.id, "🐑 Мее! Дружок, постриги сначала свою овечку.", show_alert=True)
        else:
            money = u[2] * 10
            update_user(u[0], paws=u[1] + money, wool=0)
            bot.edit_message_text(f"💰 Ты успешно продал всю шерсть! Получено: {money} 🐾", call.message.chat.id, call.message.message_id,
                                  reply_markup=types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("⬅️ Назад", callback_data='back')))
        ...
        if u[1] < 200:
            bot.answer_callback_query(call.id, "❌ Недостаточно копытц!", show_alert=True)
        else:
            res = random.random() * 100
            if res <= 2: # 2%
                name, emoji, rar = random.choice([("Радиоактивная овечка", "☢️"), ("Пустотная овечка", "👁️‍🗨️")]), "Космическая ⚫️"
            elif res <= 10: # 8%
                name, emoji, rar = random.choice([("Священная овечка", "👼"), ("Призрачная овечка", "👻")]), "Мифическая 🔴"
            elif res <= 25: # 15%
                name, emoji, rar = random.choice([("Магмовая овечка", "🔥"), ("Бриллиантовая овечка", "💎")]), "Легендарная 🟡"
            elif res <= 50: # 25%
                name, emoji, rar = random.choice([("Шизанутая овечка", "💥"), ("Милая овечка", "🎀")]), "Эпическая 🟣"
            else: # 50%
                name, emoji, rar = random.choice([("Деревенская овечка", "🏡"), ("Пляжная овечка", "🏖️")]), "Редкая 🔵"
            
            update_user(u[0], paws=u[1]-200, mut_name=name[0] if isinstance(name, tuple) else name, 
                        mut_emoji=name[1] if isinstance(name, tuple) else emoji, rarity=rar)
            bot.edit_message_text(f"🥚 Ты открыл яйцо! Тебе выпала:\n{rar} {name}", call.message.chat.id, call.message.message_id,
                                  reply_markup=types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("⬅️ Назад", callback_data='back')))
