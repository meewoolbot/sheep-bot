...
def admin_give_paws(message):
    if message.from_user.id == ADMIN_ID:
        try:
            ...
            update_user(int(target_id), paws=u[1] + int(amount))
            bot.send_message(message.chat.id, f"✅ Выдано {amount} 🐾")
        except: bot.send_message(message.chat.id, "Ошибка. Формат: `/give_paws ID кол-во`")
