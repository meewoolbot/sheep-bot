...
def get_user(user_id):
    ...
    if not row:
        ...
        return u
    return {
        "id": row[0], "skin_name": row[1], "skin_emoji": row[2], 
        "balance": row[3], "wool": row[4], "next_harvest": row[5], 
        "is_shearing": row[6], "shear_finish": row[7]
    }
...
app = Flask(__name__)
@app.route('/')
def index(): return "Овечья ферма онлайн! 🐑"
...
async def sell(cb: types.CallbackQuery):
    ...
    if u['wool'] <= 0: await cb.answer("🐑 Мее! Сначала постриги овечку.", show_alert=True)
        ...
        m = u['wool'] * 10; u['balance'] += m; u['wool'] = 0; save_user(u)
        await cb.message.edit_text(f"💰 Продано! Получено: {m} 🐾", reply_markup=InlineKeyboardBuilder().add(types.InlineKeyboardButton(text="⬅️ Назад", callback_data="back")).as_markup())
