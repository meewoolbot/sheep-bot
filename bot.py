...
def get_user(user_id):
    ...
    conn.close()
    if not row:
        user = {"id": user_id, "skin_name": "Обычная овечка", "skin_emoji": "🐑", "balance": 0, "wool": 0, "next_harvest": time.time(), "is_shearing": 0, "shear_finish": 0}
        save_user(user)
        return user
    return {"id": row[0], "skin_name": row[1], "skin_emoji": row[2], "balance": row[3], "wool": row[4], "next_harvest": row[5], "is_shearing": row[6], "shear_finish": row[7]}
...
app = Flask(__name__)
@app.route('/')
def index(): return "Ферма запущена! Порт 8080 активен. 🐑"
...
async def cmd_sheep(message: types.Message):
    ...
    now = time.time()
    if now >= u['next_harvest']:
        timer = "✅ Шерсть готова к сбору."
    else:
        rem = int(u['next_harvest'] - now)
        timer = f"⏳ Шерсть будет готова к сбору через: {time.strftime('%H:%M:%S', time.gmtime(rem))}"
...
async def process_shear(cb: types.CallbackQuery):
    ...
    if u['is_shearing']:
        if now >= u['shear_finish']:
            ...
            u['wool'] += gain; u['is_shearing'] = 0; u['next_harvest'] = now + 43200
            save_user(u)
            ...
            try: await cb.message.edit_text(f"✂️ Стрижём твою овечку.\n⏳ Процесс займет: {time.strftime('%M:%S', time.gmtime(rem))}", reply_markup=kb)
        ...
        u['is_shearing'] = 1; u['shear_finish'] = now + 300
        save_user(u)
        kb = InlineKeyboardBuilder().add(types.InlineKeyboardButton(text="⌛️ Проверить время", callback_data="shear")).as_markup()
...
async def sell(cb: types.CallbackQuery):
    u = get_user(cb.from_user.id)
    if u['wool'] <= 0:
        await cb.answer("🐑 Мее! Дружок, постриги сначало свою овечку.", show_alert=True)
    else:
...
async def open_egg(cb: types.CallbackQuery):
    ...
    if u['balance'] < 200:
        ...
        u['balance'] -= 200
        r_keys = list(SHEEP_RARITIES.keys())
        rarity = random.choices(r_keys, weights=[SHEEP_RARITIES[k]["weight"] for k in r_keys])[0]
        name, emoji = random.choice(SHEEP_RARITIES[rarity]["items"])
        u['skin_name'], u['skin_emoji'] = f"{rarity}: {name}", emoji
        save_user(u)
        kb = InlineKeyboardBuilder().add(types.InlineKeyboardButton(text="⬅️ Назад", callback_data="back")).as_markup()
        await cb.message.edit_text(f"🥚 Ты открыл яйцо! Тебе выпала:\n{u['skin_name']} {u['skin_emoji']}", reply_markup=kb)
