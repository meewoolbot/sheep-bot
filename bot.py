...
def get_profile_data(u):
    ...
    if u[7] == 0:
        if diff >= timedelta(hours=12):
            ...
            h, m = divmod(m, 60)
            status_msg = f"⏳ Шерсть будет готова к сбору через: {h:02d}:{m:02d}:{s:02d}"
    elif u[7] == 1:
        status_msg = "✅ Шерсть готова к сбору."
...
def sheep_cmd(message):
    u = get_user(message.from_user.id)
    if not u: return
    text, markup = get_profile_data(u)
...
def admin_give(message):
    if message.from_user.id == ADMIN_ID:
        try:
            ...
            curr = get_user(int(uid))
            update_user(int(uid), paws=curr[1] + int(amt))
            bot.send_message(message.chat.id, f"✅ Баланс +{amt} 🐾")
        except: pass
...
def handle_query(call):
    ...
    if call.data == 'shear':
        if u[7] == 0: 
            ...
            update_user(u[0], status=2, last_action=now.strftime('%Y-%m-%d %H:%M:%S'))
            markup = types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("⌛️ Проверить время", callback_data='check_time'))
            bot.edit_message_text("✂️ Стрижём твою овечку. ⏳ Процесс займет: 5:00", call.message.chat.id, call.message.message_id, reply_markup=markup)
        elif u[7] == 2:
            markup = types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("⌛️ Проверить время", callback_data='check_time'))
            bot.edit_message_text("✂️ Стрижка продолжается...", call.message.chat.id, call.message.message_id, reply_markup=markup)

    elif call.data == 'check_time':
        diff = now - last_t
        if diff >= timedelta(minutes=5):
            gain = random.randint(5, 15)
            update_user(u[0], status=0, wool=u[2] + gain, last_action=now.strftime('%Y-%m-%d %H:%M:%S'))
            ...
            time_str = f"{m}:{s:02d}"
            markup = types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("⌛️ Проверить время", callback_data='check_time'))
            bot.edit_message_text(f"✂️ Стрижём твою овечку. ⏳ Осталось ждать: {time_str}", call.message.chat.id, call.message.message_id, reply_markup=markup)

    elif call.data == 'market_eggs':
        markup = types.InlineKeyboardMarkup(row_width=1).add(types.InlineKeyboardButton("🥚 Открыть яйцо", callback_data='open_egg'), types.InlineKeyboardButton("⬅️ Назад", callback_data='back_to_profile'))
        ...
        if u[1] < 200: bot.answer_callback_query(call.id, "🐑 Дружок, у тебя тут не хватает!", show_alert=True)
        else:
            ...
            update_user(u[0], paws=u[1]-200, mut_name=n, mut_emoji=e, rarity=r)
            bot.edit_message_text(f"🥚 Ты успешно открыл яйцо! Тебе выпала: {e} {n}", call.message.chat.id, call.message.message_id, reply_markup=types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("⬅️ Назад", callback_data='market_eggs')))

    elif call.data == 'market_wool':
        markup = types.InlineKeyboardMarkup(row_width=1).add(types.InlineKeyboardButton("💰 Продать шерсть", callback_data='sell_wool'), types.InlineKeyboardButton("⬅️ Назад", callback_data='back_to_profile'))
        ...
        if u[2] <= 0: bot.answer_callback_query(call.id, "🐑 Дружок, ты сначало овечку постриги!", show_alert=True)
        else:
            gain = u[2] * 10
            update_user(u[0], paws=u[1] + gain, wool=0)
            bot.edit_message_text(f"💰 Ты успешно продал шерсть! Получено: {gain} 🐾", call.message.chat.id, call.message.message_id, reply_markup=types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("⬅️ Назад", callback_data='market_wool')))

    elif call.data == 'back_to_profile':
        u_upd = get_user(call.from_user.id)
        text, markup = get_profile_data(u_upd)
