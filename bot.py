...
def sheep_profile(message):
    ...
    if u[7] == 0:
        if diff >= timedelta(hours=12):
            ...
            status_text = f"⏳ Готовность через: {str(rem).split('.')[0]}"
    elif u[7] == 1: status_text = "✅ Шерсть готова к сбору."
    elif u[7] == 2:
        if diff >= timedelta(minutes=5): status_text = "✅ Стрижка завершена!"
        else:
...
def admin_panel(message):
    if message.from_user.id == ADMIN_ID:
        bot.send_message(message.chat.id, "👑 `/give_paws ID кол-во`", parse_mode='Markdown')
...
def admin_give_paws(message):
    if message.from_user.id == ADMIN_ID:
        try:
            ...
            u = get_user(int(tid))
            if u:
                update_user(int(tid), paws=u[1] + int(amt))
                bot.send_message(message.chat.id, f"✅ Выдано {amt} 🐾")
...
def handle_query(call):
    ...
    if call.data == 'shear':
        if u[7] == 0: bot.answer_callback_query(call.id, "❌ Овечка еще не готова!", show_alert=True)
        elif u[7] == 1:
            update_user(call.from_user.id, status=2, last_action=now.strftime('%Y-%m-%d %H:%M:%S'))
            bot.edit_message_text("✂️ Стрижём... Процесс: 5 мин.", call.message.chat.id, call.message.message_id, 
                                  reply_markup=types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("⌛️ Проверить", callback_data='check_shear')))
        ...
        if (now - last_t) >= timedelta(minutes=5):
            ...
            bot.edit_message_text(f"🐑 Готово! Получено: {gain} 🧶", call.message.chat.id, call.message.message_id, 
                                  reply_markup=types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("⬅️ Назад", callback_data='back')))
        else: bot.answer_callback_query(call.id, "Еще стрижём...")
        ...
        bot.edit_message_text(f"💰 Курс 1=10. У тебя: {u[2]} 🧶", call.message.chat.id, call.message.message_id, reply_markup=markup)

    elif call.data == 'sell':
        if u[2] <= 0: bot.answer_callback_query(call.id, "Мее! Нет шерсти.", show_alert=True)
        else:
            money = u[2] * 10
            update_user(call.from_user.id, paws=u[1] + money, wool=0)
            bot.edit_message_text(f"💰 Продано на {money} 🐾", call.message.chat.id, call.message.message_id, reply_markup=types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("⬅️ Назад", callback_data='back')))

    elif call.data == 'buy_eggs':
        markup = types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("🥚 Открыть (200 🐾)", callback_data='open_egg'), types.InlineKeyboardButton("⬅️ Назад", callback_data='back'))
        bot.edit_message_text(f"🥚 Баланс: {u[1]} 🐾", call.message.chat.id, call.message.message_id, reply_markup=markup)

    elif call.data == 'open_egg':
        if u[1] < 200: bot.answer_callback_query(call.id, "❌ Мало копытц!", show_alert=True)
        else:
            res = random.random() * 100
            if res <= 2: n, e, r = random.choice([("Радиоактивная", "☢️"), ("Пустотная", "👁️‍🗨️")]), "Космическая ⚫️"
            elif res <= 10: n, e, r = random.choice([("Священная", "👼"), ("Призрачная", "👻")]), "Мифическая 🔴"
            elif res <= 25: n, e, r = random.choice([("Магмовая", "🔥"), ("Бриллиантовая", "💎")]), "Легендарная 🟡"
            elif res <= 50: n, e, r = random.choice([("Шизанутая", "💥"), ("Милая", "🎀")]), "Эпическая 🟣"
            else: n, e, r = random.choice([("Деревенская", "🏡"), ("Пляжная", "🏖️")]), "Редкая 🔵"
            update_user(call.from_user.id, paws=u[1]-200, mut_name=n, mut_emoji=e, rarity=r)
            bot.edit_message_text(f"🥚 Выпала: {e} {n}\n🧬 {r}", call.message.chat.id, call.message.message_id, reply_markup=types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("⬅️ Назад", callback_data='back')))
